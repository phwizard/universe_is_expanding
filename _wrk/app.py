...
# Session state for sentence and history
if "current_sentence" not in st.session_state:
    st.session_state["current_sentence"] = "The universe is expanding."

if "history" not in st.session_state:
    st.session_state["history"] = []

sentence = st.text_input("Enter a sentence to explore:", st.session_state["current_sentence"])

if st.session_state["history"] == [] or sentence != st.session_state["history"][-1]:
...
if st.button("Explore"):
    ...
    if not valid_ideas:
        ...
        for i, idea in enumerate(valid_ideas):
            ...
            if st.button(f"🔁 Expand: {idea}", key=f"expand_{i}"):
                st.session_state["current_sentence"] = idea
